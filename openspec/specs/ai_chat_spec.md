...
```
  ...
  ("Lá bưởi bị đốm vàng")
         |
         v
  [1. Shared Embedding Service] ----> [2. Gemini text-embedding-004]
         |                                  |
  ...
  (System Prompt + Context Chunks + User Message + Chat History)
         |
         v
  [6. Google Gemini Flash API] ----> [7. Save to MongoDB (chat_sessions)]
         |
         v
  Safe UI Response
...
*   **Payload DTO (`SendMessageDto`):**
    ...
    export class SendMessageDto {
      ...
      @IsOptional()
      session_id?: string; // Nếu trống, backend tự khởi tạo Session mới
    }
    ```
*   **Response (200 OK):**
    ```json
      ...
      "data": [
        {
          ...
          "created_at": "2026-05-18T01:10:00Z",
          "updated_at": "2026-05-18T01:13:00Z"
        }
      ],
      "pagination": {
...
*   **Payload DTO (`SubmitFeedbackDto`):**
    ...
    export class SubmitFeedbackDto {
      ...
      @IsNotEmpty()
      session_id: string;
      message_id: string; // ID subdocument assistant trong chat_sessions
...
2.  **Nếu `flashKey` > 14 (Flash quá tải):** Trả fallback ngay với `rate_limited: true`, hoặc HTTP 429 cho endpoint không hỗ trợ fallback. Không đưa chat realtime vào BullMQ.
3.  **Nếu Gemini trả HTTP 429/5xx:** Kích hoạt **Exponential Backoff Retry** ngắn (1s, 2s, 4s; tối đa 3 lần).
4.  **Nếu retry vẫn thất bại:** Trả fallback tĩnh hoặc HTTP 429 theo `onRateLimit`; kết thúc SSE hiện tại. BullMQ chỉ dùng cho embedding, insight, reminder và tác vụ background.
...
- [ ] **TC-CHAT-01:** Tạo hội thoại mới không truyền `session_id` -> Kết quả: Hệ thống tự sinh Session ID mới trong MongoDB `chat_sessions` và lưu lượt chat.
...
- [ ] **TC-CHAT-04:** Spam gửi tin nhắn liên tục (> 15 requests trong 1 phút từ nhiều client) -> Kết quả: Hệ thống trả fallback/429 ngay, không tạo job trong BullMQ.
- [ ] **TC-CHAT-05:** Gửi đánh giá feedback AI hợp lệ -> Kết quả: Ghi nhận thành công vào MongoDB collection `ai_feedback` để phục vụ cải tiến prompt.
