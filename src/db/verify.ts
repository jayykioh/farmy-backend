...
async function bootstrap() {
  ...
  try {
    ...
    const farmPlotModel = app.get<Model<FarmPlotDocument>>(getModelToken(FarmPlotDocument.name));
    const diaryModel = app.get<Model<DiaryDocument>>(getModelToken(DiaryDocument.name));
    const diaryLogModel = app.get<Model<DiaryLogDocument>>(getModelToken(DiaryLogDocument.name));
    const reminderModel = app.get<Model<ReminderDocument>>(getModelToken(ReminderDocument.name));
    const migrationModel = app.get<Model<MigrationDocument>>(getModelToken(MigrationDocument.name));

    console.log('\n========================================================================');
    console.log('                 DATABASE CONTENT VERIFICATION RESULTS                  ');
    console.log('========================================================================');
    ...
    plots.forEach((p) => console.log(`  - Name: ${p.name}, Area: ${p.area_size}m², Owner ID: ${p.user_id}`));

    const diaries = await diaryModel.find().exec();
    console.log(`\n📓 Diaries (${diaries.length}):`);
    diaries.forEach((d) => console.log(`  - Crop: ${d.crop_type}, Status: ${d.status}, Start: ${d.start_date.toLocaleDateString()}`));

    const logs = await diaryLogModel.find().exec();
    console.log(`\n📝 Diary Logs (${logs.length}):`);
    logs.forEach((l) => console.log(`  - Activity: ${l.activity_type}, Content: "${l.content}", Embedding Dimensions: ${l.content_embedding?.length}`));

    const reminders = await reminderModel.find().exec();
    console.log(`\n⏰ Reminders (${reminders.length}):`);
    reminders.forEach((r) => console.log(`  - Title: "${r.title}", Remind At: ${r.remind_at.toLocaleString()}, Sent: ${r.is_sent}`));
