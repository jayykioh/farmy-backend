...
export class EmbeddingRepository {
  ...
  async findActiveChunkStates(sourceId: string, sourceType: string): Promise<Map<number, { contentHash: string }>> {
    ...
    await this.dataSource.query(
      `INSERT INTO "embeddings" ("source_id", "source_type", "chunk_index", "content_hash", "embedding", "metadata", "is_active")
       ...
       DO UPDATE SET 
         ...
         "created_at" = now()`,
      [dto.sourceId, dto.sourceType, dto.chunkIndex, dto.contentHash, vectorString, metadataString, isActive],
    );
  }

  async upsertMany(dtos: UpsertEmbeddingDto[], staleIndices: number[] = []): Promise<void> {
