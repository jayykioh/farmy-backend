...
describe('ChunkingService', () => {
  ...
  describe('chunkText', () => {
    it('should split text into chunks based on window size without overlap if step = windowSize', () => {
      ...
      const result = chunkingService.chunkText(text, { windowSize: 3, stepSize: 3 });
    ...
    it('should handle overlaps correctly when stepSize < windowSize', () => {
      ...
      const result = chunkingService.chunkText(text, { windowSize: 4, stepSize: 2 });
      expect(result).toEqual(['abcd', 'cdef', 'efgh', 'ghij']);
    });

    it('should handle text shorter than windowSize', () => {
      const text = 'abc';
      const result = chunkingService.chunkText(text, { windowSize: 5, stepSize: 3 });
      expect(result).toEqual(['abc']);
    });

    it('should return empty array for empty string', () => {
      const result = chunkingService.chunkText('', { windowSize: 5, stepSize: 3 });
      expect(result).toEqual([]);
    });

    it('should throw error if windowSize is less than or equal to 0', () => {
      expect(() => chunkingService.chunkText('abc', { windowSize: 0, stepSize: 1 })).toThrow('windowSize must be greater than 0');
    });

    it('should throw error if stepSize is less than or equal to 0', () => {
      expect(() => chunkingService.chunkText('abc', { windowSize: 5, stepSize: 0 })).toThrow('stepSize must be greater than 0');
    });

    it('should throw error if stepSize is greater than windowSize', () => {
      expect(() => chunkingService.chunkText('abc', { windowSize: 5, stepSize: 6 })).toThrow('stepSize cannot be greater than windowSize');
    });
  });
});
