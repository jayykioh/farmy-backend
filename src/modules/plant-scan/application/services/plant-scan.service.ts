...
export class PlantScanService {
  ...
  async diagnose(file: any, cropType: string, userId: string, tier: string = 'free') {
    ...
    if (isBlurry) {
      throw new HttpException(
        { success: false, statusCode: 422, errorCode: 'SCAN_IMAGE_BLURRY', message: 'Ảnh mờ, chụp lại' },
        HttpStatus.UNPROCESSABLE_ENTITY,
      );
    }

    // 4. Optimize image & compute pHash
    const optimizedBuffer = await this.imageProcessor.optimizeImage(file.buffer);
    ...
    if (!rpmLimit.allowed || !rpdLimit.allowed) {
      throw new HttpException(
        { success: false, statusCode: 429, errorCode: 'AI_SCAN_QUOTA_BUSY', message: 'Hệ thống AI đang quá tải. Vui lòng thử lại sau vài phút.' },
        HttpStatus.TOO_MANY_REQUESTS,
      );
    }
    ...
    try {
      ...
      return await this.saveFailedScan(userId, cropType, imageKey, thumbnailKey, pHash, 'LLM_ERROR', startedAt);
    }
    ...
    try {
      ...
      return await this.saveFailedScan(userId, cropType, imageKey, thumbnailKey, pHash, 'INVALID_JSON', startedAt);
    }
    ...
    if (!validationResult.success) {
      ...
      return await this.saveFailedScan(userId, cropType, imageKey, thumbnailKey, pHash, 'INVALID_SCHEMA', startedAt);
    }
    ...
    if (diagnosisResult.is_plant === false) {
      await this.saveFailedScanRecord(userId, cropType, imageKey, thumbnailKey, pHash, 'NOT_A_PLANT_IMAGE', startedAt);
      throw new HttpException(
        { success: false, statusCode: 422, errorCode: 'NOT_A_PLANT_IMAGE', message: 'Ảnh không phải là cây trồng' },
        HttpStatus.UNPROCESSABLE_ENTITY,
      );
    }

    // 11. Apply Safety Guardrails & warnings
    const finalDiagnosis = this.guardrailService.applyBVTVGuardrail(diagnosisResult);
    ...
    try {
      ...
      this.logger.error('Failed to save scan to MongoDB. Deleting R2 objects...', e);
      await this.storageService.deleteFile(imageKey);
      await this.storageService.deleteFile(thumbnailKey);
      throw new HttpException(
        { success: false, statusCode: 500, errorCode: 'PLANT_SCAN_PERSISTENCE_FAILED', message: 'Lỗi lưu trữ dữ liệu' },
  ...
  private async saveFailedScanRecord(userId: string, cropType: string, imageKey: string, thumbnailKey: string, pHash: string, errorCode: string, startedAt: number) {
    const scanDoc = new this.scanModel({
  ...
  private async saveFailedScan(userId: string, cropType: string, imageKey: string, thumbnailKey: string, pHash: string, errorCode: string, startedAt: number) {
    await this.saveFailedScanRecord(userId, cropType, imageKey, thumbnailKey, pHash, errorCode, startedAt);
    throw new HttpException(
      { success: false, statusCode: 500, errorCode, message: 'Hệ thống AI gặp sự cố' },
      HttpStatus.INTERNAL_SERVER_ERROR,
    );
  }

  async getScanById(scanId: string, userId: string) {
    const scan = await this.scanModel.findOne({ _id: scanId, user_id: userId }).exec();
    if (!scan) {
      throw new HttpException(
        { success: false, statusCode: 404, errorCode: 'SCAN_NOT_FOUND', message: 'Không tìm thấy kết quả quét' },
