...
export class IdempotencyExecutionService {
  ...
  async acquireOrTakeoverLock(
    ...
    const existing = await this.executionModel.findOne({ userId, idempotencyKey }).exec();
    ...
    if (!existing) {
      try {
        ...
        if (error.code === 11000) {
          return this.acquireOrTakeoverLock(userId, idempotencyKey, requestHash, leaseSeconds);
        }
        throw error;
      }
    }
    ...
    if (existing.status === 'processing' && existing.leaseUntil > now) {
      throw new HttpException(
        { message: 'Request in progress', errorCode: 'IDEMPOTENCY_IN_PROGRESS' },
        HttpStatus.CONFLICT
      );
    }
    ...
    if (!updated) {
      throw new HttpException(
        { message: 'Concurrent lock takeover conflict', errorCode: 'IDEMPOTENCY_IN_PROGRESS' },
        HttpStatus.CONFLICT
      );
    }
